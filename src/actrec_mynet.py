...
layers = {'ff': ('param_init_fflayer', 'fflayer'),
          ...
          'lstm':('param_init_lstm','lstm_layer'),# add by kong, the standard LSTM
          }
...
def lstm_cond_layer(tparams, state_below, options, prefix='lstm',
    ...
    def _step(m_, x_, h_, c_, a_, ct_, dp_=None, dp_att_=None):
        ...
        # attention
        # print '\n\ncheck\n\n'
        pstate_ = tensor.dot(h_, tparams[_p(prefix,'Wd_att')]) # pstate_         Song Sijie equation(2) Wd*ht-1
        pctx_ = tensor.dot(x_, tparams[_p(prefix,'Wc_att')]) + tparams[_p(prefix, 'b_att')]    # Wc*x + b
        if options['n_layers_att'] > 1:
            for lidx in xrange(1, options['n_layers_att']):
                pctx_ = tensor.dot(pctx_, tparams[_p(prefix,'W_att_%d'%lidx)])+tparams[_p(prefix, 'b_att_%d'%lidx)]
                if lidx < options['n_layers_att'] - 1:
                    pctx_ = tanh(pctx_)
        pctx_ = pctx_ + pstate_[:,None,:]        #Song Sijie equation(2) Wd*h_t-1 + Wc*x + b
        pctx_list = []
        pctx_list.append(pctx_)
        pctx_ = tanh(pctx_)          #Song Sijie equation(2)   tanh(Wd*h_t-1 + Wc*x + b)
        alpha = tensor.dot(pctx_, tparams[_p(prefix,'U_att')])+tparams[_p(prefix, 'c_tt')]  #Song Sijie equation(2)   U*tanh(Wd*ht-1 + Wc*x + b) + c
        alpha_pre = alpha
        alpha_shp = alpha.shape
        # Song Sijie equation(2)   softmax(U*tanh(Wd*ht-1 + Wc*x + b) + c)
        alpha = tensor.nnet.softmax(options['temperature_inverse']*alpha.reshape([alpha_shp[0],alpha_shp[1]])) # softmax this is important!!kong 7*7 weights
        ctx_ = (x_ * alpha[:,:,None]).sum(1) # current context
        # print '\n\ncheck\n\n'
        if options['selector']:
            # This is for one scalar.       beta_t = sigmoid(Ws*h_t-1 + bs)
            sel_ = tensor.nnet.sigmoid(tensor.dot(h_, tparams[_p(prefix, 'W_sel')])+tparams[_p(prefix,'b_sel')])
            sel_ = sel_.reshape([sel_.shape[0]])   # This is for beta kong Why no softmax?
            ctx_ = sel_[:,None] * ctx_
        ...
        c = m_[:,None] * c + (1. - m_)[:,None] * c_   # mask is 0 or not ,if 0, keep the state unchange
    ...
    #sel_ = tensor.nnet.softmax(sel_) #add by kong. To softmax the temporal attention
...
def build_model(tparams, options):
    ...
    opt_outs = dict()
    opt_outs['hidden'] = proj_h #my hidden
    if options['selector']:
...
def train(dim_out=100, # hidden layer dim for outputs
          ...
          dataset='', # dummy dataset, replace with video ones
    ...
    tparams = init_tparams(params) #init or read saved model weight pkl
    ...
    for epochidx in xrange(max_epochs):
        ...
        for tbidx in xrange(num_train_batches):
            ...
            if numpy.mod(uidx, saveFreq) == 0:
                ...
                numpy.savez(saveto+str(uidx), history_errs=history_errs, **params)

                pkl.dump(model_options, open('%s%d.pkl'%(saveto,uidx), 'wb'))
                print 'Done'
