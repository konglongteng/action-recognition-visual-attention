...
class DataHandler(object):
  ...
  def __init__(self, data_pb):
    ...
    self.Reset()
    self.batch_data_  = np.zeros((self.batch_size_,self.seq_length_,  self.frame_size_), dtype=np.float32) # 4keras
    if data_pb.dataset != 'h2mAP':
      self.batch_label_ = np.zeros((self.batch_size_), dtype=np.int64) #self.seq_length_,
    else:
      self.batch_label_ = np.zeros((self.batch_size_, 12), dtype=np.int64) #self.seq_length_,
    self.handler = data
  ...
  def GetBatch(self, data_pb, verbose=False):
    ...
    for j in xrange(batch_size):
      ...
      if length >= self.seq_length_*skip:
          ...
          self.batch_data_[j,:n, :] = self.handler[start:start+length:skip, :]
          self.batch_data_[j,n:, :] = np.tile(self.batch_data_[n-1,j, :],(self.seq_length_-n,1))
      if data_pb.dataset != 'h2mAP':
        #self.batch_label_[:,j] = np.tile(label,(1,self.seq_length_))
        self.batch_label_[j] = label
      else:
    ...
    if data_pb.dataset=='ucf11':
      ...
      self.batch_data_ = self.batch_data_.reshape([self.batch_data_.shape[0],self.batch_data_.shape[1],49,1024]).astype('float32')
    elif data_pb.dataset=='imageshdf':
      self.batch_data_ = self.batch_data_.reshape([self.batch_data_.shape[0],self.batch_data_.shape[1],224,224,3]).astype('float32')
  ...
  #   elif data_pb.dataset=='h2mAP':
  #     data_ = data_.reshape([data_.shape[0],data_.shape[1],49,1024]).astype('float32')
  #   elif data_pb.dataset=='volley_lstm':
  #     data_ = data_.reshape([data_.shape[0],data_.shape[1],1024]).astype('float32')
  #   elif data_pb.dataset=='volley_group_lstm':
  #     data_ = data_.reshape([data_.shape[0], data_.shape[1],2048]).astype('float32')
  #
  #   f.close()
  #
  #   ### label_
...
class TrainProto(object):
  def __init__(self, bs, maxlen, stride, dataset, fps=30):
    ...
    self.fps = fps
    if dataset=='imageshdf':
      self.data_file       = '/home/kong/extDisk/train_images.h5'
      self.num_frames_file = '/home/kong/extDisk/train_imagenum.txt'
      self.labels_file     = '/home/kong/extDisk/train_imagelabels.txt'
      self.vid_name_file   = '/home/kong/extDisk/train_imagename.txt'
...
class TestTestProto(object):
  def __init__(self, bs, maxlen, stride, dataset, fps=30):
    ...
    if dataset=='imageshdf':
      self.data_file = '/home/kong/extDisk/test_images.h5'
      self.num_frames_file = '/home/kong/extDisk/test_imagenum.txt'
      self.labels_file = '/home/kong/extDisk/test_imagelabels.txt'
      self.vid_name_file = '/home/kong/extDisk/test_imagename.txt'
      self.dataset_name = 'features'
...
def test():
  fps = 30
  data_pb = TrainProto(5,10,1,'imageshdf',fps)
